...
async function main() {
  ...
  // Programmatic SEO landing pages under /use-cases/<doc-slug>/<vertical-slug>/.
  // Generated by scripts/build-use-cases.mjs (run before this script via
  // npm "build"). Two-level directory tree; copy recursively.
  const ucSrc = join(SRC, 'use-cases');
  const ucOut = join(OUT, 'use-cases');
  if (await exists(ucSrc)) {
    ...
    for (const docEntry of await readdir(ucSrc, { withFileTypes: true })) {
      ...
      for (const vertEntry of await readdir(docDir, { withFileTypes: true })) {
        ...
        const vertOut = join(docOut, vertEntry.name);
        await mkdir(vertOut, { recursive: true });
        for (const f of await readdir(vertDir)) {
          await copyFile(join(vertDir, f), join(vertOut, f));
        }
        count++;
      }
    }
    console.log(`  wrote ${count} use-case landing pages to dist/use-cases/`);
  }
  ...
  const draftSrc = join(SRC, 'draft');
  if (await exists(draftSrc)) {
    const draftOut = join(OUT, 'draft');
    await mkdir(draftOut, { recursive: true });
    for (const f of ['index.html', 'app.js', 'draft.css']) {
      await copyFile(join(draftSrc, f), join(draftOut, f));
      console.log(`  wrote dist/draft/${f}`);
    }
  }

  // /templates-data.json: the full owned-library manifest the /templates/ page
  // fetches to render all 500+ cards + build its category filter. Generated by
  // scripts/build-templates-library.mjs. Without this in dist the page silently
  // falls back to its tiny built-in stub list, so a missing manifest must FAIL
  // the build rather than ship a broken library page.
  const tmplDataSrc = join(SRC, 'templates-data.json');
  if (!(await exists(tmplDataSrc))) {
    throw new Error(
      'web/templates-data.json is missing. The /templates/ library page would ' +
      'silently fall back to its tiny built-in stub list. Run ' +
      '`node scripts/build-templates-library.mjs` to regenerate it from the catalog.',
    );
  }
  await copyFile(tmplDataSrc, join(OUT, 'templates-data.json'));
  console.log('  wrote dist/templates-data.json');

  // /templates-pdf/<slug>.pdf: pre-rendered contract template PDFs served via the
  ...
  if (!(await exists(pdfSrc))) {
    throw new Error(
      'web/templates-pdf/ is missing. Every template download would 404 and fall ' +
      'back to a generated wireframe. Run `node scripts/build-templates-library.mjs` ' +
      'to render the template PDFs.',
    );
  }
  const pdfOut = join(OUT, 'templates-pdf');
  await mkdir(pdfOut, { recursive: true });
  let pdfCount = 0;
  for (const f of await readdir(pdfSrc)) {
    if (!f.toLowerCase().endsWith('.pdf')) continue;
    await copyFile(join(pdfSrc, f), join(pdfOut, f));
    pdfCount++;
  }
  if (pdfCount === 0) {
    throw new Error(
      'web/templates-pdf/ contains zero PDFs. Every template download would 404 and ' +
      'fall back to a generated wireframe. Run `node scripts/build-templates-library.mjs` ' +
      'to render the template PDFs.',
    );
  }
  console.log(`  wrote ${pdfCount} template PDFs to dist/templates-pdf/`);

  // /templates-all.zip: one-click "download all" bundle of every rendered
  // template PDF (built by scripts/build-templates-library.mjs companion zip step).
  // Best-effort: warn if missing but don't fail the build.
  const tmplZipSrc = join(SRC, 'templates-all.zip');
  if (await exists(tmplZipSrc)) {
