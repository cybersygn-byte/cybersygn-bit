...
top 10; visibility falls off after the first few hours so the upvote
push needs to be front-loaded.

## Tagline (60-char max, the chip under product name)

```
The e-signature tool that finds every field for you.
```

Alternates (rotate if early traction is weak):
- `Sign PDFs without dragging boxes onto every page.`
- `30 minutes in DocuSign → 3 seconds with CyberSygn.`
- `Drop a PDF. Sign it. Done. No field placement needed.`

## Description (260-char hero)

```
CyberSygn locates every signature line, initial, date, and checkbox in
your PDFs automatically. No drag-and-drop, no signer accounts, no
nonsense. Send a contract in 3 seconds, get it back signed with an
audit certificate. Built solo on Cloudflare. Free for 3 documents.
```

## First comment from the maker (the most important comment of the day)
...
For every contract. That broke me.
...
- [ ] PH account verified, gradient ready (have shipped previous things, or accept being a first-timer)
...
## Launch day (the first 6 hours)

| Time (PT) | Action |
|---|---|
| 12:01 am | Post goes live. First comment from maker (above) within 5 minutes. |
...
- [ ] Send a "thank you" email to every Charter signup that came via PH
- [ ] Reply to every PH comment that arrived overnight
- [ ] Publish a "what I learned launching solo on PH" Indie Hackers post (huge SEO + community goodwill)
- [ ] Submit to the PH-adjacent directories (BetaList, Tekpon, AlternativeTo, SaasHub)
